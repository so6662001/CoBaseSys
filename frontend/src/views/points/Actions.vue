<template>
  <CrudTable title="积分动作" :data="list" :loading="loading" :total="total" :page="page" :pageSize="pageSize"
    ...
    <el-table-column prop="direction" label="方向" width="80">
      <template #default="{ row }">
        <el-tag :type="row.direction === 1 ? 'success' : 'danger'" size="small">{{ row.direction === 1 ? '增加' : '扣减' }}</el-tag>
      </template>
    </el-table-column>
  ...
  <el-dialog v-model="dialogVisible" :title="isEdit ? '编辑动作' : '新增动作'" width="500px">
    <el-form :model="form" label-width="90px">
      ...
      <el-form-item label="方向">
        <el-select v-model="form.direction"><el-option :value="1" label="增加" /><el-option :value="-1" label="扣减" /></el-select>
      </el-form-item>
...
const form = reactive({ systemId: 1, actionCode: '', actionName: '', description: '', direction: 1, status: 1 })
...
async function fetchData() {
  ...
  try {
    ...
    const res = await pointApi.listActions(params)
...
function openDialog(row) {
  ...
  Object.assign(form, row || { systemId: 1, actionCode: '', actionName: '', description: '', direction: 1, status: 1 })
...
async function handleSubmit() {
  ...
  try {
    if (isEdit.value) await pointApi.updateAction(editId.value, form)
    else await pointApi.createAction(form)
    ElMessage.success(isEdit.value ? '更新成功' : '创建成功')
    dialogVisible.value = false; fetchData()
  } finally { submitting.value = false }
}

async function handleDelete(row) { await pointApi.deleteAction(row.id); ElMessage.success('删除成功'); fetchData() }
