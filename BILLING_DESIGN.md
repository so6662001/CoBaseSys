...
### 3.8 月费配额套餐 (`QUOTA_PLAN`)

```
┌─────────────────────────────────────────────┐
│  月费固定 + 多维度用量配额                       │
│  超过配额 → 必须升级到更高套餐才能继续使用         │
│  不允许超量使用（硬上限）                        │
└─────────────────────────────────────────────┘
```
...
**核心特点：**
- 按月固定费用，到期按月续费
- 每个套餐定义多维度的配额上限（如客户数、提货次数、API调用次数等）
- 配额维度可自定义，通过 JSON 定义 `quota_config`
- 超过任何一个维度的配额则无法继续使用，必须升级套餐
- 支持免费版（月费=0）

**定价参数：**
- `period_price`：月费（分）
- `period_type`：固定为 MONTH
- `quota_config`：JSON 格式的多维度配额配置

```json
{
  "quotas": [
    {"dimension": "customer_count", "label": "客户数", "limit": 50},
    {"dimension": "delivery_count", "label": "提货次数/月", "limit": 200}
  ]
}
```
...
**外部系统查询当前用量是否超限：**
- `POST /api/v1/billing/check/quota` — 查询指定维度的已用量和剩余量
- 返回 `{allowed: true/false, used: 35, limit: 50, remaining: 15}`
...
- [x] **数据模型**：14 张核心表字段设计 — **已确认**
- [x] **赠送规则**：买 N 送产品/套餐/积分三种类型 — **已确认**
- [x] **折扣规则**：产品折扣、满额折扣、满额送积分 — **已确认**
- [x] **积分抵扣**：按比例上限 + 汇率换算 — **已确认**
- [x] **续费定价**：价格锁定到到期日 — **已确认**
- [x] **提醒策略**：到期前15天、每天14:00、短信+应用内 — **已确认**
- [x] **支付方式**：余额/微信/支付宝/线下转账 — **已确认**
- [x] **数据库**：双数据源（PostgreSQL + MySQL） — **已确认**
- [x] **外部 API**：到期查询和用量上报 — **已确认**
- [x] **性能**：Redis缓存 + 虚拟线程 + 读写分离 — **已确认**
- [x] **开发顺序**：后端API → 管理后台前端 → 客户H5前台 — **已确认**
- [x] **腾讯云SMS**：已确认使用腾讯云短信 — **已确认**

---

**所有设计事项已全部确认，可以开始编码开发。**
