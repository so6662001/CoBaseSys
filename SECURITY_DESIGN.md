...
│  分布式锁: billing:reconcile:incr:lock                 │
│                                                      │
│  任务A: 最近2小时新增流水的哈希链校验                     │
│    ├─ 只校验 created_at >= now()-2h 的记录              │
│    └─ 发现断裂 → 立即告警 (短信+邮件+企业微信)           │
│                                                      │
│  任务B: 最近2小时有变动的账户余额摘要校验                  │
│    ├─ 只校验 updated_at >= now()-2h 的账户              │
│    └─ 不一致 → 立即告警 + 冻结相关账户                   │
│                                                      │
│  ═══ 全量对账：每天凌晨 3:00 ═══                        │
│  分布式锁: billing:reconcile:full:lock                  │
│  锁超时: 60分钟                                        │
│                                                      │
│  任务1: 哈希链完整性全量校验                              │
...
│                                                      │
│  告警规则:                                            │
│  ├─ 签名失败 > 100次/分钟 → 短信+企业微信告警 + 临时封禁IP│
│  ├─ 管理员登录失败 > 5次 → 锁定账号 + 短信通知          │
│  ├─ 单账户积分变动 > 10万/小时 → 企业微信告警审核        │
│  ├─ 单账户余额变动 > 5万元/小时 → 企业微信告警审核       │
│  ├─ 对账任务发现异常 → 立即告警(短信+企业微信) + 冻结    │
│  ├─ 数据库慢查询 > 5秒 → 运维告警                      │
│  └─ 数据库连接数 > 80% → 运维告警                      │
└──────────────────────────────────────────────────────┘
```
...
| **P0** | 审计日志独立存储 (独立MySQL实例) | 一期 | 日志被篡改 |
| **P2** | 字段级加密 | 二期 | 数据泄露 |
| **P2** | 异地灾备 | 二期 | 灾难恢复 |
...
- [x] **哈希链方案**：流水表通过 SHA256 哈希链串联 — **已确认**
- [x] **只追加账本**：数据库触发器禁止 UPDATE/DELETE，冲正通过反向交易 — **已确认**
- [x] **对账频率**：每 2 小时增量对账 + 每天凌晨 3:00 全量对账 — **已确认**
- [x] **管理员认证**：用户名+密码+JWT+MFA — **已确认**
- [x] **RBAC角色**：超级管理员/运营主管/销售员/财务/客服/审计员 — **已确认**
- [x] **MFA方式**：短信验证码 + 企业微信 — **已确认**（原TOTP改为企业微信）
- [x] **审计日志保留**：主库90天、审计库3年、归档永久 — **已确认**
- [x] **备份策略**：每日全量+实时增量+异地备份 — **已确认**
- [x] **告警渠道**：短信 + 邮件 + 企业微信 — **已确认**（原钉钉改为企业微信）
- [x] **独立审计库**：部署独立审计数据库实例 — **已确认，一期实施**
